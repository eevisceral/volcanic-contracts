...
// Lava.
contract Lava is ERC20('Volcanic Token', 'LAVA') {
...
contract LavaMasterChef is Ownable, ReentrancyGuard {
    ...
    event UpdateEmissionRate(address indexed user, uint256 LavaPerBlock);
    ...
    event LavaMintError(bytes reason);
    ...
    function pendingLava(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            accLavaPerShare = accLavaPerShare.add(LavaReward.mul(1e18).div(pool.lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        try token.mint(devaddr, LavaReward.div(10)) {
        } catch (bytes memory reason) {
            LavaReward = 0;
            emit LavaMintError(reason);
        }

        try token.mint(address(this), LavaReward) {
        } catch (bytes memory reason) {
            LavaReward = 0;
            emit LavaMintError(reason);
        }

        pool.accLavaPerShare = pool.accLavaPerShare.add(LavaReward.mul(1e18).div(pool.lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function safeLavaTransfer(address _to, uint256 _amount) internal {
        uint256 LavaBal = token.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > LavaBal) {
            transferSuccess = token.transfer(_to, LavaBal);
        } else {
    ...
    function updateEmissionRate(uint256 _LavaPerBlock) external onlyOwner {
        require(_LavaPerBlock <= MAX_EMISSION_RATE, "INCORRECT INPUT");
        massUpdatePools();
        LavaPerBlock = _LavaPerBlock;
